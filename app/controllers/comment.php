<?php
    ...
    switch ($action) {
        ...
        case "edit" :
            ...
            if (empty($theResDate)) $theResDate = $rdate;
            $logDesc = $p->getentDesc();
            if ($theFollowup == 'on') { $followup = 'checked'; } else { $followup = '';}
            if ($theResolved == 'on') { $resolved = 'checked'; } else { $resolved = '';}
            $content = "    <label>Initial Log Entry:<br><textarea id=\"initEntryText\" name=\"initEntryText\" rows=\"6\" cols=\"75\" style=\"background-color: silver\" readonly>$logDesc</textarea></label><br>\n";
            $content .= "    <label>Comment(s):<br><textarea id=\"Plan\" name=\"Plan\" class=\"editor\" rows=\"6\" cols=\"75\">$thePlan</textarea></label><br>\n";
            $content .= "    <label>STAFF or DEPARTMENT Responsible:<br><input id=\"Responsible\" name=\"Responsible\" size=\"50\" maxlength=\"50\" value=\"$theResponsible\"></label><br>\n";
            $content .= "    <label>Follow-Up  Required:&nbsp;</label>&nbsp;<input id=\"Followup\" name=\"Followup\" type=\"checkbox\" $followup>&nbsp;Checked = Yes<br>\n";
            $content .= "    <label>Resolved:&nbsp;</label>&nbsp;<input id=\"Resolved\" name=\"Resolved\" type=\"checkbox\" $resolved>&nbsp;Checked = Resolved on: $theResDate<br>\n";
            $content .= "    <input type=\"hidden\" name=\"ResDate\" value=\"$rdate\" />\n";
            $content .= "    <input type=\"hidden\" name=\"initEntry\" value=\"$id\" />\n";
            $content .= "    <input type=\"hidden\" name=\"nid\" value=\"" . $q->getId() . "\" />\n";
            return (array('content' => $content, 'action' => 'EditPlan', 'myaction' => 'Update Comments', 'tinymce'=>$tinymce));
        ...
        $content .=
            "    <label>Comment(s):<br><textarea id=\"Plan\" name=\"Plan\" class=\"editor\" rows=\"6\" cols=\"75\">$thePlan</textarea></label><br>\n";;
        $content .=
       ...
       $content .=
        ...
        return ( [ 'content' => $content, 'action' => 'AddPlan', 'myaction' => 'Create Comments', 'tinymce'=>$tinymce ] );
